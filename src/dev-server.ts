...
const downloadBinary = async (version: string, cacheDir: string): Promise<string> => {
  ...
  if (fs.existsSync(binaryPath) && fs.existsSync(versionFile)) {
    ...
    if (cachedVersion === version) {
      console.log(
        `[QStash Dev][DEBUG] downloadBinary: v${version} already cached, skipping download`
      );
      return binaryPath;
    }
    console.log(
      `[QStash Dev][DEBUG] downloadBinary: outdated v${cachedVersion}, upgrading to v${version}`
    );
