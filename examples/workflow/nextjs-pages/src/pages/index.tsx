...
function Home() {
  ...
  return (
    ...
    <div className='flex flex-col justify-between h-screen '>
      ...
      <div className="flex flex-row items-center justify-center flex-grow gap-8 text-black bg-gray-100">
        <div className='w-160'>
        ...
        <button
          onClick={handleSend}
          disabled={loading}
          className={`w-full px-4 py-2 text-white rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 ${loading ? 'bg-gray-500' : 'bg-emerald-500 hover:bg-emerald-600'}`}
        >
          {loading ? 'Sending...' : 'Send request to /api/path'}
        </button>
      </div>
      </div>
